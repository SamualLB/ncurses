require "../ncurses"

NCurses.start
NCurses.cbreak
NCurses.no_echo
NCurses.no_timeout

left = NCurses::Window.new(NCurses.height, NCurses.width / 2, 0, 0)
right = NCurses::Window.new(NCurses.height, NCurses.width / 2, 0, NCurses.width - (NCurses.width / 2))

left.print "Left side!\n"
left.print "Width: #{left.width}\n"
left.print "Height: #{left.height}\n"

right.print "Right side!\n"
right.print "Width: #{right.width}\n"
right.print "Height: #{right.height}\n"

left.refresh
right.refresh

sleep 5

left.clear
left.resize NCurses.height / 2, NCurses.width
left.print "Top side!\n"
left.print "Width: #{left.width}\n"
left.print "Height: #{left.height}\n"

right.clear
right.resize NCurses.height / 2, NCurses.width
right.move_window NCurses.height - (NCurses.height / 2), 0
right.print "Bottom side!\n"
right.print "Width: #{right.width}\n"
right.print "Height: #{right.height}\n"

left.refresh
right.refresh

sleep 5

NCurses.end
